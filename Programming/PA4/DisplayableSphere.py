...
Extended EBO to support normal mapping
...
class DisplayableSphere(Displayable):
    ...
    def generate(self, radius=1, stacks=18, slices=36, color=None):
        ...
        self.vtxCoord = np.zeros([stacks, slices, 9])
        for i in range(stacks):
            ...
            for j in range(slices):
                ...
                self.vtxCoord[i, j, :3] = [
                                            ...
                                            self.radius * math.cos(phi) * math.cos(theta),
                                            self.radius * math.sin(phi),
                                            self.radius * math.cos(phi) * math.sin(theta)
                                          ]
                # du
                self.vtxCoord[i, j, 3:6] = [
                                            # swap du.y and du.z accordingly
                                            -self.radius * math.cos(phi) * math.sin(theta),
                                            0,
                                            self.radius * math.cos(phi) * math.cos(theta)
                                          ]
                # dv
                self.vtxCoord[i, j, 6:9] = [
                                            # swap du.y and du.z accordingly
        ...
        # add one more slices to avoid a gap on texture image
        self.vertices = np.zeros([(stacks) * (slices+1), 17])
        self.vertices[:,6:9] = [*color]

        for i in range(stacks):
            for j in range(slices+1):
                [x,y,z,xu,yu,zu,xv,yv,zv] = self.vtxCoord[i%stacks,j%slices,:]
                # vertex coords
                self.vertices[i * (slices+1) + j,0:3] = [x,y,z]
                # vertex normal
                self.vertices[i * (slices+1) + j,3:6] = [x,y,z]
                # vertex texture coords
                self.vertices[i * (slices+1) + j,9:11] = [1-j/slices,i/(stacks-1)]
                # du
                self.vertices[i * (slices+1) + j,11:14] = [xu,yu,zu]
                # dv
                self.vertices[i * (slices+1) + j,14:17] = [xv,yv,zv]
